...
public class Helpers {
  public static boolean isAllianceHubActive() {
    final double matchTime = DriverStation.getMatchTime(); // Time remaing in either an Auto period, or a Tele-Op period
    ...
    final String gameData = DriverStation.getGameSpecificMessage(); // Either an "R" or "B", based on who won Auto
    ...
    final Alliance gameDataAlliance = gameData.equals("R") ? Alliance.Red : Alliance.Blue; // Is the game data R?

    return !(isAllianceShiftEven^(myAlliance.get() == gameDataAlliance)); // If the shift is even and you won Auto or it's odd and you lost Auto
  }

  private static boolean getIsAllianceShiftEven(double matchTime) {
    matchTime -= 30.0; // Take off EndGame
    if (matchTime > 50.0) {
      matchTime -= 50.0; // Only checking if it's even, so only need to consider on each half of "the 100" seconds
    }
